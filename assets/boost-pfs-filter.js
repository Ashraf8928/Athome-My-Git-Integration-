...
var boostPFSFilterConfig = {
  general: {
    ...
    shortenPipsRange: true,
  },
  selector: {
    otpButtons: ".boost-pfs-filter-product-item-image",
    avoidStickyHeader: ".header--root.sticky",
  },
};
...
(function () {
  ...
  boostPFSFilterConfig.general.separateRefineByFromFilter =
    ...
    boostPFSThemeConfig.custom.filter_tree_horizontal_style != "style-expand"
      ? true
      : false;
  Selector.stickyElementDesktop =
    jQ(".boost-pfs-filter-tree-v").length > 0 && !Utils.isMobile()
  ...
  ProductGridItem.prototype.compileTemplate = function (data) {
    ...
    itemHtml = itemHtml.replace(
      /{{itemSwatches}}/g,
      buildProductOptionSwatches(data)
    );
    ...
    itemHtml = itemHtml.replace(/{{itemVId}}/g, firstVariant.id);
    // let imgURL = `/collections/${boostPFSAppConfig.general.collection_handle}${Utils.buildProductItemUrlWithVariant(data)}`
    let imgURL = buildCustomProductItemUrlWithVariant(data);
    itemHtml = itemHtml.replace(/{{imgURL}}/g, imgURL);
    console.log(imgURL);
  ...
  //Boost #163981
  function buildCustomProductItemUrlWithVariant(data) {
    // Check if is split product by variant
    var variantIdPart =
      data.split_product && data.variants
        ? "?variant=" + data.variants[0].id
        : "";
    var productUrl = Utils.buildProductItemUrl(data, true); // #163981
  ...
  function buildImages(data) {
    ...
    for (var i = 0; i < rangeWidths.length; i++) {
      ...
      if (activeSwapImage) {
        flipImageSrcSet +=
          Utils.optimizeImage(images[1]["src"], rangeWidths[i] + "x") +
          " " +
          rangeWidths[i] +
          "w";
      }
    ...
    html +=
      '<a href="{{imgURL}}" class="boost-pfs-filter-product-item-image-link" aria-label="{{itemTitle}}" ';
  ...
  function buildPrice(data) {
    ...
    if (
      ...
      html = '<div class="boost-pfs-filter-product-item-price">';
      if (onSale) {
        // html += '<s>' + 'MRP:&nbsp;' + Utils.formatMoney(data.compare_at_price_min) + '</s> ';
        html +=
          '<span class="boost-pfs-filter-product-item-sale-price plp_oriprice">' +
          Utils.formatMoney(data.price_min) +
          "</span>";
        html +=
          '<div class="boost-pfs-filter-product-item-price plp_onsale"> ' +
        ...
        if (priceVaries) {
          html +=
            ...
            boostPFSThemeConfig.label_basic.from +
            " " +
            "</span>";
        }
        html +=
          '<span class="boost-pfs-filter-product-item-regular-price sandesh">' +
          Utils.formatMoney(data.price_min) +
          "</span>";
      }
      html += "</div>";
  ...
  function buildLabels(data) {
    ...
    var soldOutLabel = "";
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("sold_out_enable") &&
      boostPFSThemeConfig.custom.sold_out_enable &&
      soldOut
    ) {
    ...
    if (boostPFSThemeConfig.custom.sale_label_enable && onSale && !soldOut) {
      ...
      saleLabel = boostPFSTemplate.saleLabelHtml.replace(
        /percent/g,
        "-" + salePercent + "%"
      );
    }
  ...
  function buildTagLabels(data, showall) {
    ...
    if (data.tags) {
      if (data.tags.includes("pfs:label-Online-Exclusive"))
        tagLabel = boostPFSTemplate.tagLabelHtml.replace(
          /{{labelTag}}/g,
          "Online Exclusive"
        );
  ...
  function buildProductOptionSwatches(data) {
    ...
    if (boostPFSThemeConfig.custom.swatch_enable) {
      ...
      for (var i = 0; i < swatchApplyFor.length; i++) {
        var optionNames = boostPFSThemeConfig.custom[
          "swatch_by_" + swatchApplyFor[i] + "_apply"
        ]
          .split(",")
          .filter(onlyUnique);
        var swatchShape =
          boostPFSThemeConfig.custom[
            "swatch_by_" + swatchApplyFor[i] + "_shape"
          ];
        ...
        for (var j = 0; j < optionNames.length; j++) {
          ...
          var dataImgSize = "360",
            bgSize = "50x",
            dataImgSrc = Utils.getFeaturedImage(
              data.images_info,
              dataImgSize + "x"
            ),
            swatchName = "#ffffff",
          ...
          if (
            ...
            if (swatchArr.length > 0) {
              ...
              swatchesProductOptionHtml +=
                '<ul class="boost-pfs-filter-item-swatch boost-pfs-filter-item-swatch-option' +
                optionName +
                " boost-pfs-filter-item-swatch-shape-" +
                swatchShape +
                " boost-pfs-filter-item-swatch-type-" +
                swatchType +
                '">';
                ...
                if (swatchType) {
                  switch (swatchType) {
                    case "img":
                      ...
                      break;
                    case "pro_img":
                      bgImageSrc = Utils.getFeaturedImage(
                        data.images_info,
                        bgSize
                      );
                      if (sImageIndex != "") {
                      ...
                      if (
                        bgImageSrc &&
                        bgImageSrc.includes("boost-pfs-no-image")
                      )
                        bgImageSrc = "";
                      break;
                    default:
  ...
  ProductPaginationDefault.prototype.compileTemplate = function (totalProduct) {
    ...
    var totalPage = Math.ceil(
      totalProduct / Utils.stripHtml(Globals.queryParams.limit)
    );
  ...
  ProductSorting.prototype.compileTemplate = function () {
    var html = "";
    if (
      boostPFSThemeConfig.custom.show_sort_by &&
      boostPFSTemplate.hasOwnProperty("sortingHtml")
    ) {
      var sortingArr = Utils.getSortingList();
      var isMobile = Utils.isMobile();
      var template = isMobile
        ? boostPFSTemplate.sortingHtml
        : boostPFSTemplate.sortingDesktopHtml;
      var activeClass = "";
  ...
  */
  ...
  ProductSorting.prototype.bindEvents = function () {
    var _this = this;
    jQ(
      ".boost-pfs-filter-filter-dropdown a, .boost-pfs-sorting-container a"
    ).click(function (e) {
      e.preventDefault();
    ...
    jQ(document).on("click", function (e) {
      if (
        jQ(e.target)
          .parents(".boost-pfs-filter-top-sorting")
          .find(".boost-pfs-filter-filter-dropdown").length === 0
      ) {
        jQ(".boost-pfs-filter-filter-dropdown")
          .hide()
          .parent()
  ...
  ProductDisplayType.prototype.compileTemplate = function () {
    ...
    if (boostPFSThemeConfig.custom.product_item_type == "grid") {
      if (
        boostPFSThemeConfig.custom.view_as_type ==
          "view_as_type_list_grid_multi_col" &&
        !Utils.isMobile()
      ) {
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "grid", "list") +
          '" title="' +
          buildDisplayTitle("list") +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-list" data-view="list"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid", 2) +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid {{class.productDisplayType}}-grid-2" data-view="grid-2"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid", 3) +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid {{class.productDisplayType}}-grid-3" data-view="grid-3"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid", 4) +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid {{class.productDisplayType}}-grid-4" data-view="grid-4"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid", 5) +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid {{class.productDisplayType}}-grid-5" data-view="grid-5"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid", 6) +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid {{class.productDisplayType}}-grid-6" data-view="grid-6"><span class="icon-fallback-text"></span></a>';
      } else {
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "list", "grid") +
          '" title="' +
          buildDisplayTitle("grid") +
          '" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid" data-view="grid"><span class="icon-fallback-text"></span></a>';
        itemHtml +=
          '<a href="' +
          Utils.buildToolbarLink("display", "grid", "list") +
          '" title="' +
  ...
  function buildDisplayTitle(type, count) {
    ...
    if (typeof count === "undefined" || count === 0)
      return Labels.gridView || "Grid view";
    return (Labels.gridViewColumns || "Grid view {{ count }} Columns").replace(
      /{{ count }}/g,
      count
    );
  }
        ...
        if (
          ...
          this.$element
            ...
            .each(function () {
              ...
              var $cssNames = jQ(".boost-pfs-filter-top-display-type")
                .attr("class")
                .split(" ");
              var $activeClass = $cssNames[$cssNames.length - 1];
              var indexCurrentColumn =
                $activeClass.split("-")[$activeClass.split("-").length - 1];

              if (
                $parent.hasClass("boost-pfs-filter-view-as-click") &&
                jQ(this).data("view") == $activeClass
              ) {
                ...
                if (jQ(".boost-pfs-filter-product-item").length > 0) {
                  var arrClass = jQ(".boost-pfs-filter-product-item")
                    .attr("class")
                    .split(" ");
                  var prevClass = "";
                  for (var i = 0; i < arrClass.length; i++) {
                    if (
                      arrClass[i].match(
                        /(^|\s)boost-pfs-filter-grid-width-\S+/g
                      )
                    ) {
                  ...
                  jQ(".boost-pfs-filter-product-item").addClass(
                    "boost-pfs-filter-grid-width-" + indexCurrentColumn
                  );
  ...
  Filter.prototype.afterRender = function (data) {
    ...
    if (data.total_product == 0) {
      jQ(".boost-pfs-filter-default-toolbar-inner").addClass(
        "boost-pfs-filter-toolbar-product-0"
      );
    } else {
      jQ(".boost-pfs-filter-default-toolbar-inner").removeClass(
        "boost-pfs-filter-toolbar-product-0"
      );
    }
    ...
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("layout_type") &&
      boostPFSThemeConfig.custom.layout_type == "fullwidth" &&
      !Utils.isMobile()
    ) {
    ...
    document.addEventListener("scroll", function () {
      if (
        jQ(".boost-pfs-filter-tree-h.boost-pfs-filter-stick").length > 0 &&
        jQ(window).width() > 767
      ) {
        jQ(".boost-pfs-filter-custom-sorting .boost-pfs-filter-filter-dropdown")
          .hide()
          .parent()
          .removeClass("boost-pfs-filter-sort-active");
      }
    ...
    if (jQ(".site-nav__thumb-menu").length > 0 && Utils.isMobile()) {
      ...
      jQ(".boost-pfs-filter-tree-mobile-button-stick-wrapper")
        .css("bottom", heightHeaderThemeStick)
        .addClass("boost-pfs-filter-sticky-bottom");
      jQ(".boost-pfs-filter-mobile-style1").addClass(
        "boost-pfs-filter-mobile-style1-sticky-bottom"
      );
    }

    if (
      typeof iwishCheckColl !== "undefined" &&
      typeof iwishCheckColl === "function"
    ) {
      iwishCheckColl();
    }
  };
  ...
  var original_onClickEventProductDisplayType =
    ProductDisplayType.prototype._onClickEvent;
  ProductDisplayType.prototype._onClickEvent = function (event) {
    ...
    if (
      ...
      jQ(".boost-pfs-filter-top-display-type").addClass(
        "boost-pfs-filter-view-as-click"
      );
      var currentClass = jQ(event.target).data("view");
      jQ(".boost-pfs-filter-top-display-type")[0].className = jQ(
        ".boost-pfs-filter-top-display-type"
      )[0].className.replace(/(^|\s)grid-\S+/g, "");
    ...
    if (aspect_ratio != "false") {
      ...
      if (aspect_ratio != "false" && aspect_ratio != "auto") {
        ...
        var ratioWidthHeightOther =
          boostPFSThemeConfig.custom.aspect_ratio_other;
  ...
  jQ(document).ready(function () {
    ...
    jQ(
      ".boost-pfs-filter-custom-drawer-close, .boost-pfs-filter-custom-drawer-overlay"
    ).on("keydown", (event) => {
    ...
    if (jQ(".mm-fixed-top").length > 0) {
      ...
      jQ(".boost-pfs-filter-collection-header-wrapper").css(
        "margin-top",
        headerFixedHeight
      );
    }
  ...
  FilterOptionItem.prototype.beforeRender = function () {
    if (this.label == "Under ₹ 10,000") {
      this.label = "₹0 - 10,000";
    }
  };

  jQ(document).on("click", ".boost-pfs-sorting-close-button", function () {
    jQ(this).parent().removeClass("boost-pfs-filter-sort-active");
  });

  /* end-boost-custom */

  FilterOptionRangeSlider.prototype.beforeRender = function () {
    this.shortenPipsRange = true;
  };
})();
